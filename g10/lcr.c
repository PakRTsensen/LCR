 ...
 * Copyright (C) 2015-2022 Hasanur Rahevy
 *
...
my_strusage( int level )
{
  ...
  switch( level ) {
      ...
      case 11: p = "LCR (Linux Cryptography Research)";
	break;
	...
	break;
      ...
      case 1:
      case 40:	p =
	    _("Usage: LCR [options] [files] (-h for help)");
	break;
      case 41:	p =
	    _("Syntax: LCR [options] [files]\n"
...
main (int argc, char **argv)
{
	...
	break;
      ...
      case aQuickAddKey: {
    const char *x_fpr, *x_quantity_str, *x_algo, *x_usage, *x_expire;
    int x_quantity = 1; // Default quantity = 1

    if (argc < 1 || argc > 5)
        wrong_args("--quick-addkey FINGERPRINT [QUANTITY [ALGO [USAGE [EXPIRE]]]");

    // Parse fingerprint
    x_fpr = *argv++; argc--;

    // Parse quantity (opsional)
    if (argc) {
        x_quantity_str = *argv++; argc--;
        char *endptr;
        x_quantity = strtol(x_quantity_str, &endptr, 10);
        
        // Validasi angka
        if (*endptr != '\0' || x_quantity <= 0) {
            fprintf(stderr, "ERROR: QUANTITY must be positive integer\n");
            exit(EXIT_FAILURE);
        }
    }

    // Parse parameter opsional: algo, usage, expire
    x_algo = "";
    x_usage = "";
    x_expire = "";
    ...
    if (mopt.forbid_gen_key)
        ...
        // Loop generate sebanyak quantity
